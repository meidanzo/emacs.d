...
There are currently two Chinese input schemes available: one is emacs-rime, and the other is pyim. Both can be considered built-in input methods. Choose one of them to use.
*** emacs-rime
Please note emacs-rime is already built into this setup. You need not install it through ELPA.

To use [[https://github.com/DogLooksGood/emacs-rime][emacs-rime]], you must first install [[https://github.com/DogLooksGood/emacs-rime/blob/master/INSTALLATION.org][librime]]. Additionally, you must use your own input scheme and reconfigure the value of =rime-user-data-dir= =rime-share-data-dir= in =init-rime.el=. This scheme differs from pyim.

After installing emacs-rime, you can use the Chinese input method in insert mode. You can construct your own input method using appropriate assertions in init-rime.el.

Please note that this repository has undergone significant changes since forking from [[https://github.com/redguardtoo/emacs.d][redguardtoo]]. It no longer uses pyim; The relevant enabling code has been commented out. If you wish to use it, uncomment the enabling code in init-chinese.el. 
...
The configuration for supporting =LaTeX-mode= is provided by =init-LaTeX-mode.el=. It has been tested only on Linux; other operating systems have not yet been verified.

It should be noted that, due to differences in plugin support for various modes, =plain-tex-mode= and =doctex-mode= are currently not supported. If needed, please configure them manually.

=init-LaTeX-mode.el= supports previewing LaTeX files. In a GUI environment, to automatically preview mathematical formulas within the =proof= environment, you need to remove =proof= from =TeX-fold-env-spec-list=; otherwise, you must manually run the =preview-latex= command to enable preview.

This configuration is tightly integrated with =latexmk=, and the PDF viewer is invoked via =latexmk=. To use it without modification, add the following to =~/.latexmkrc=:
...
If Emacs is started as a daemon, add the following to your =zathurarc= to enable PDF forward and inverse search:
...
For more detailed implementation and design explanations, please refer to the comments in =init-LaTeX-mode.el= and the documentation of the related plugins.
